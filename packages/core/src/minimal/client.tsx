...
const DEFAULT_HTML_HEAD = [
  ...
  <meta
    ...
    key="viewport"
  />,
  <meta name="generator" content="Render" key="generator" />,
];

const BASE_RSC_PATH = `${import.meta.env?.RENDER_CONFIG_BASE_PATH}${
  import.meta.env?.RENDER_CONFIG_RSC_BASE
}/`;
...
export const unstable_fetchRsc = (
  ...
  if (import.meta.hot) {
    ...
    (globalThis as any).__RENDER_RSC_RELOAD_LISTENERS__ ||= [];
    const index = (globalThis as any).__RENDER_RSC_RELOAD_LISTENERS__.indexOf(
      (globalThis as any).__RENDER_REFETCH_RSC__!,
    );
    if (index !== -1) {
      (globalThis as any).__RENDER_RSC_RELOAD_LISTENERS__.splice(index, 1, refetchRsc);
    } else {
      (globalThis as any).__RENDER_RSC_RELOAD_LISTENERS__.push(refetchRsc);
    }
    (globalThis as any).__RENDER_REFETCH_RSC__ = refetchRsc;
  }
